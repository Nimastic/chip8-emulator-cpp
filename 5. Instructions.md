CHIP-8 has 34 instructions that we need to emulate.

Opcode list is referenced by austinmorlan's guide who references Cowgod
...
The top of the stack has the address of one instruction past the one that called the subroutine, so we can put that back into the PC
this overwrites our pc += 2
void Chip8::OP_00EE()
{
	--sp;
	pc = stack[sp];
...
1nnn: JP addr
Jump to address nnn
Interpreter sets the the pc to nnn
...
When we call a subroutine, we want to return eventually, so we put current PC+2 into stack
PC += 2 is done in Cycle(), 
so the current PC holds the next instruction after this CALL
void Chip8::OP_2nnn()
{
	uint16_t address = opcode & 0x0FFFu;
...
since PC+2 in cycle(), we can just PC+2 again to skip next
void Chip8::OP_3xkk()
{
