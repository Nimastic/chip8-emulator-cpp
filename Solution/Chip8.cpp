/*
 * Single-file CHIP-8 emulator based on Austin Morlan's tutorial:
 * https://austinmorlan.com/posts/chip8_emulator/
 *
 * Build (MSYS2 / MinGW example):
 *   g++ -std=c++17 Chip8.cpp -o chip8 -lSDL2
 *
 * Run:
...
Chip8::Chip8()
	: randGen(static_cast<unsigned int>(
...
void Chip8::OP_Dxyn()
{
	...
	for (unsigned int row = 0; row < height; ++row)
	{
		...
		for (unsigned int col = 0; col < 8; ++col)
		{
			...
			unsigned int screenY = (yPos + row) % VIDEO_HEIGHT;
			unsigned int screenX = (xPos + col) % VIDEO_WIDTH;
...
int main(int argc, char** argv)
{
	...
	int videoPitch = static_cast<int>(sizeof(chip8.video[0]) * VIDEO_WIDTH);
